...
class Ctrl{
	...
	wechatSignIn(req, res, next) {
	  if(this.app.get('env') == "development") {
	    const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjViMDdiMWZjYjgzNWE2MzI2YzNjMmYxZiIsImlhdCI6MTUyNzIzNDI4OCwiZXhwIjozNzUyNzIzNDI4OH0.0wEbxORm2ZcuuUbDrJ6urfkvhus-5hWCpbEuJMjoEy4'
		...
		.then(doc => {
          ...
          password: res.jwt.setMd5('123456'), 
			  ...
			  return this.model.newAndSave(body)
			}
		...
		.then(doc => {
      if (doc && doc._id) return res.tools.setJson(0, '注册并登录成功', {
        token: res.jwt.setToken(doc._id)
      })
    })
		.catch(err => next(err))
...
//		const signature = req.body.signature
	...
	signIn(req, res, next) {
		...
		if (this.app.get('env') != 'development')
      if (req.body.super == true && username != config.superAdmin.username)  return res.tools.setJson(1, '此用户无权登录！')
